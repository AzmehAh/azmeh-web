...
import { supabase } from "../lib/supabase"; // تأكد من مسار الإتصال بالـ Supabase
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      ...
      const { data, error } = await supabase
        ...
        .select("*")
        .eq("id", id)
        .single(); // مهم جدا: .single() لأننا نتوقع منتج واحد
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <section className="py-24 bg-gradient-to-br from-[#2C5DB6] to-[#1e4080] text-white">
        <div className="container mx-auto px-6 lg:px-20">
          {/* Breadcrumb */}
          <div className="flex items-center text-sm text-white mb-8">
            <Link to="/">Home</Link>
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              ...
              <p className="text-blue-100/90 mb-6">{product.technicalDescription}</p>
              ...
              <button
                ...
                className="bg-gradient-to-r from-yellow-400 to-yellow-500 hover:from-yellow-500 hover:to-yellow-600 text-black px-8 py-4 rounded-xl font-bold shadow-lg flex items-center transition-transform hover:scale-105"
              >
      ...
      {product.technicalSpecs.length > 0 && (
        <section className="py-16 bg-white">
          <div className="container mx-auto px-6 lg:px-20 bg-white rounded-2xl shadow-lg overflow-hidden mb-16">
      ...
      <section className="py-16 bg-white">
        <div className="container mx-auto px-6 lg:px-20">
          ...
          <div className="max-w-4xl mx-auto relative">
            ...
            {product.instructions.map((instr, index) => (
              <motion.div
                key={index}
        ...
        <div className="container mx-auto px-6 lg:px-20 grid lg:grid-cols-2 gap-8">
          ...
          <div className="bg-white rounded-2xl shadow-lg overflow-hidden">
            ...
            <div className="p-6 space-y-3">
              {product.safety.precautions.map((prec, index) => (
                <div key={index} className="flex items-start">
                  <div className="w-2 h-2 bg-red-500 rounded-full mt-2 mr-4 flex-shrink-0" />
                  <p className="text-gray-700">{prec}</p>
                </div>
              ))}
            </div>
          </div>
