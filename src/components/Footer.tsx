import React from 'react';
import { Mail, Phone, MapPin, Facebook, Instagram, Linkedin, Youtube } from 'lucide-react';
import { Link } from 'react-router-dom';
import { useTranslation } from 'react-i18next';

const Footer = () => {
  const { t, i18n } = useTranslation();
  const isRTL = i18n.language === 'ar';

  // لعكس ترتيب الأعمدة في RTL
  const columns = [
    {
      id: 'company',
      content: (
        <div>
          <div className={`flex items-center mb-6 ${isRTL ? 'justify-end' : 'justify-start'}`}>
            <img
              src="/images/Azmeh-Paints-Logo.png"
              alt="Al Azmeh Paints"
              className="h-12 w-auto filter brightness-0 invert"
            />
          </div>
          <p className="text-white text-sm leading-relaxed">
            {t('footer.companyDescription')}
          </p>
          <div className="pt-4">
            <p className="text-white text-sm">
              {t('footer.copyright', { year: new Date().getFullYear() })}
            </p>
          </div>
        </div>
      ),
    },
    {
      id: 'links',
      content: (
        <div>
          <h4 className="font-semibold mb-6 text-white text-lg">
            {t('footer.quickLinks')}
          </h4>
          <ul className={`space-y-3 ${isRTL ? 'pr-2' : 'pl-2'}`}>
            <li><Link to="/" className="hover:text-gray-200 text-sm">{t('common.home')}</Link></li>
            <li><Link to="/products" className="hover:text-gray-200 text-sm">{t('common.products')}</Link></li>
            <li><Link to="/blog" className="hover:text-gray-200 text-sm">{t('common.blog')}</Link></li>
            <li><Link to="/about" className="hover:text-gray-200 text-sm">{t('common.about')}</Link></li>
            <li><Link to="/contact" className="hover:text-gray-200 text-sm">{t('common.contact')}</Link></li>
          </ul>
        </div>
      ),
    },
    {
      id: 'contact',
      content: (
        <div>
          <h4 className="font-semibold mb-6 text-white text-lg">
            {t('footer.contactInfo')}
          </h4>
          <div className="space-y-4">
            <div className={`flex items-start ${isRTL ? 'flex-row-reverse' : ''}`}>
              <Mail className={`w-5 h-5 text-white mt-0.5 flex-shrink-0 ${isRTL ? 'ml-3' : 'mr-3'}`} />
              <p className="text-gray-300 text-sm">manager@dkl-syria.com</p>
            </div>
            <div className={`flex items-start ${isRTL ? 'flex-row-reverse' : ''}`}>
              <Phone className={`w-5 h-5 text-white mt-0.5 flex-shrink-0 ${isRTL ? 'ml-3' : 'mr-3'}`} />
              <p className="text-gray-300 text-sm">(+963) 988 691 712</p>
            </div>
            <div className={`flex items-start ${isRTL ? 'flex-row-reverse' : ''}`}>
              <MapPin className={`w-5 h-5 text-white mt-0.5 flex-shrink-0 ${isRTL ? 'ml-3' : 'mr-3'}`} />
              <p className="text-gray-300 text-sm">{t('footer.address')}</p>
            </div>
          </div>
        </div>
      ),
    },
    {
      id: 'social',
      content: (
        <div>
          <h4 className="font-semibold mb-6 text-white text-lg">
            {t('footer.followUs')}
          </h4>
          <div className={`flex gap-4 ${isRTL ? 'flex-row-reverse' : ''}`}>
            {[Facebook, Instagram, Linkedin, Youtube].map((Icon, i) => (
              <a
                key={i}
                href="#"
                className="w-10 h-10 bg-gray-800 rounded-full flex items-center justify-center hover:bg-[#2C5DB6] transition-colors duration-300"
              >
                <Icon className="w-5 h-5" />
              </a>
            ))}
          </div>
          <div className="mt-6 pt-6 border-t border-gray-800">
            <p className="text-gray-400 text-xs">{t('footer.stayUpdated')}</p>
          </div>
        </div>
      ),
    },
  ];

  // عكس ترتيب الأعمدة في RTL
  const orderedColumns = isRTL ? [...columns].reverse() : columns;

  return (
    <footer className={`bg-[#0055A3] text-white ${isRTL ? 'text-right' : 'text-left'}`} dir={isRTL ? 'rtl' : 'ltr'}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        {/* استخدام flex بدلاً من grid لتحكم أفضل في الترتيب */}
        <div className={`flex flex-col ${isRTL ? 'md:flex-row-reverse' : 'md:flex-row'} gap-10`}>
          {orderedColumns.map((col) => (
            <div
              key={col.id}
              className={`flex-1 min-w-0 ${isRTL ? 'md:text-right' : 'md:text-left'}`}
            >
              {col.content}
            </div>
          ))}
        </div>

        {/* --- Bottom Section --- */}
        <div className="mt-12 pt-8 border-t border-gray-800">
          <div className={`flex flex-col md:flex-row items-center gap-4 ${isRTL ? 'md:flex-row-reverse md:justify-end' : 'md:justify-between'}`}>
            <p className="text-gray-400 text-sm text-center md:text-left">
              {t('footer.professionalSolutions')}
            </p>
            <div className={`flex gap-6 text-sm ${isRTL ? 'flex-row-reverse' : ''}`}>
              <Link to="/privacy" className="text-gray-400 hover:text-white transition-colors">
                {t('footer.privacyPolicy')}
              </Link>
              <Link to="/terms" className="text-gray-400 hover:text-white transition-colors">
                {t('footer.termsOfService')}
              </Link>
            </div>
          </div>
        </div>
      </div>
    </footer>
  );
};

export default Footer;