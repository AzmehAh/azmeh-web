            ...
            {cards.map((card) => (
              ...
              >
                ...
                <div className="absolute inset-0 bg-black/30 transition-opacity duration-300"
                  style={{ opacity: hoveredCard === card.id ? 0.8 : 0 }}
                ></div>
                ...
                <div className="relative z-10 p-6 h-full flex flex-col justify-between">
                  ...
                  {/* Expanding description with bubble effect */}
                  {hoveredCard === card.id && (
                    <motion.div
                      initial={{ scale: 0 }}
                      animate={{ scale: 1 }}
                      transition={{ duration: 0.5, ease: 'easeOut' }}
                      className="absolute inset-0 bg-white/90 rounded-xl p-4 flex items-center justify-center text-gray-800 text-sm leading-relaxed"
                    >
                      {card.description}
                    </motion.div>
                  )}
