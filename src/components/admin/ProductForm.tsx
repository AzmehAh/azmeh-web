...
const ProductForm = () => {
  ...
  const [formData, setFormData] = useState<any>({
    ...
    type_id: '',
    usage_id: '',
    material_id: [], // ← مصفوفة فارغة افتراضيًا
  });
  ...
  const fetchProduct = async () => {
    ...
    try {
      ...
      // ✅ جلب المواد من جدول product_materials فقط
      ...
      const parsed = {
        ...
        material_id: materialIds, // ← من الجدول الجديد فقط

        // الحقول النصية
        storing_conditions: productData?.storing_conditions || '',
        storing_conditions_ar: productData?.storing_conditions_ar || '',
        // ... باقي الحقول النصية موجودة في productData
    ...
    try {
      const uploaded = await Promise.all(files.map(async (file) => {
        ...
        const { error: uploadError } = await supabase.storage
          .from('products')
          .upload(`product_images/${fileName}`, file);

        if (uploadError) {
          if (uploadError.message.includes('Bucket not found')) {
            throw new Error('Storage bucket "products" not found. Please create it in Supabase Storage.');
          }
          throw uploadError;
        }
        ...
        const { data } = supabase.storage
          .from('products')
          .getPublicUrl(`product_images/${fileName}`);
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      // ✅ إعداد بيانات المنتج (بدون material_id)
      const productData = {
        name: formData.name || '',
        ...
        // العلاقات (المرتبطة بجداول أخرى)
        brand_id: formData.brand_id || null,
        type_id: formData.type_id || null,
        usage_id: formData.usage_id || null,
        // الحقول المصفوفة (يجب أن تكون JSON)
      ...
      if (!productId) throw new Error('Product ID is missing after save');

      // ✅ حفظ علاقة المواد (Many-to-Many)
      const cleanMaterialIds = (formData.material_id || [])
      ...
      if (deleteError) throw new Error('Delete old materials failed: ' + deleteError.message);

      // إدخال الروابط الجديدة
      if (cleanMaterialIds.length > 0) {
        const materialRelations = cleanMaterialIds.map((materialId: string) => ({
        ...
        if (insertError) throw new Error('Insert materials failed: ' + insertError.message);
      }
      ...
      if (images.length > 0) {
        ...
        if (imgError) throw new Error('Save images failed: ' + imgError.message);
      }
