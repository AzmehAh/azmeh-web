      ...
      <div className="flex h-full">
        {paintCategories.map((category, index) => {
          ...
          return (
            <motion.div
              key={category.id}
              className={`relative h-full cursor-pointer transition-all duration-500 ${isActive ? "flex-grow" : "flex-shrink"}`}
              initial={{ flex: 1 }}
              animate={{
                flex: isActive ? 5 : 1,
                marginLeft: isActive ? (index === paintCategories.length - 1 ? "0" : "-25px") : "-25px",
                marginRight: isActive ? (index === paintCategories.length - 1 ? "0" : "-25px") : "-25px",
              }}
              style={{
            ...
            >
              ...
              {!isActive && (
                <motion.div
                  ...
                  animate={{ opacity: 1 }}
                >
                  <p className="text-white text-lg font-semibold rotate-90 tracking-widest whitespace-nowrap">
                    {category.title}
                  </p>
                </motion.div>
              )}
              ...
              {isActive && (
                <motion.div
                  className="absolute inset-0 flex flex-col justify-center items-start p-12 z-20 bg-black/50"
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
